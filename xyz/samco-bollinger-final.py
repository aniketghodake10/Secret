...
#                     'TATASTEEL', 'TECHM', 'UPL', 'ULTRACEMCO', 'WIPRO', 'GAIL', 'COALINDIA']
...
def samco_5m_candle_data(tickerr):
    global login
    today = dt.now()
    yesterday = str(today - td(days = 3))[:19]
...
def ohlc_5m():
    ...
    dff = samco_5m_candle_data(Stock_samco1[0])
...
def bolling_macd():
    ...
    for stk in Stock_samco1:
        now = dt.now().minute
        if int(float(df8['dateTime'].iloc[-1][14:16])) != now - now%5:
        ...
        for i in range(1, 6):
            if df_low.iloc[-1 * i] < LowerBand.iloc[-1 * i] or df_high.iloc[-1 * i] > UpperBand.iloc[-1 * i]:
                baddy = 0
        for i in range(6, 8):
            if BandDiffer.iloc[-1 * i] > 0.008 or df_open.iloc[-1 * i] < LowerBand.iloc[-1 * i] or df_close.iloc[-1 * i] > UpperBand.iloc[-1 * i] or BandDiffer.iloc[-1 * i] > 0.008:
                baddy = 0
        ...
        for stk in list_cond1:
            ...
            if df_close.iloc[-1] >= 1.0005 * UpperBand.iloc[-1]:
                if df_open.iloc[-1] <= UpperBand.iloc[-1] and df_open.iloc[-1] >= MiddleBand.iloc[-1] and macd.iloc[-1] > \
                        signal.iloc[-1] and rsii.iloc[-1] < 75:
                    aniket = stk+'up'
                    break
            ...
            if df_close.iloc[-1] <= 0.9995 * LowerBand.iloc[-1]:
                if df_open.iloc[-1] >= LowerBand.iloc[-1] and df_open.iloc[-1] <= MiddleBand.iloc[-1] and macd.iloc[-1] < \
                        signal.iloc[-1] and rsii.iloc[-1] > 25:
                    aniket =stk+'do'
                    break
